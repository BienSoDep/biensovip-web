...
export default function Terms() {
  const { data: db } = usePolicyPage('terms');
  const dbContent = db ? (() => { try { return JSON.parse(db.contentJson); } catch { return null; } })() : null;
  const sections = dbContent?.sections || contentItems('terms.sections');
  const title = db?.title || contentGet('terms.title');
  const updated = db?.updatedLabel || contentGet('terms.updated');
  return (
