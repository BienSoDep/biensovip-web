...
export default function Fav({ favCards, patch, go, notify }) {
  return (
    <section style={{ maxWidth: 'var(--width-content)', margin: '0 auto', padding: 'var(--space-8) var(--pad-page) var(--pad-section-y)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'pageIn 180ms var(--ease-out)' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', gap: 'var(--space-4)' }}>
        ...
        {favCards.length > 0 && <Button variant="outline" size="sm" onClick={() => { patch({ favs: {} }); notify('Đã bỏ lưu tất cả'); }}>Bỏ lưu tất cả</Button>}
      </div>
      {favCards.length > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(276px,1fr))', gap: 'var(--gutter-section)', animation: 'fadeIn 180ms var(--ease-out)' }}>
