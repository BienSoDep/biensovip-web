  ...
  return (
    <article style={{ maxWidth: 760, margin: '0 auto', padding: 'var(--space-8) var(--pad-page) var(--pad-section-y)', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'pageIn 180ms var(--ease-out)' }} itemScope itemType="https://schema.org/BlogPosting">
      <nav aria-label="Breadcrumb" style={{ font: 'var(--type-caption)', color: 'var(--text-muted)' }}>
        Trang chủ <span aria-hidden>›</span> <button type="button" onClick={go('blog')} style={{ border: 'none', background: 'transparent', padding: 0, cursor: 'pointer', font: 'var(--type-caption)', color: 'var(--action-primary)' }}>Tin phong thủy</button>
      </nav>
      ...
      <header style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
          ...
          <span style={{ font: 'var(--type-caption)', color: 'var(--text-faint)' }}>
            <time dateTime={post.date} itemProp="datePublished">{post.date}</time> · {mins} phút đọc · <span itemProp="author">Duy Đinh</span>
          </span>
        </div>
      ...
      {heads.length > 1 && (
        <nav aria-label="Mục lục" style={{ background: 'var(--surface-sunken)', borderRadius: 'var(--radius-card)', padding: 'var(--space-4) var(--gutter-card)', display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
        ...
        if (b.t === 'plate') {
          ...
          return (
            <div key={i} style={{ background: 'var(--surface-sunken)', borderRadius: 'var(--radius-card)', padding: 'var(--gutter-card)', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 'var(--space-4)' }}>
              ...
              <Button variant="dark" size="sm" onClick={() => openPlate(p.id)}>Xem biển</Button>
            </div>
          );
        }
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)', paddingTop: 'var(--space-2)', boxShadow: 'inset 0 1px 0 var(--border-hairline)' }}>
        <Button variant="primary" size="md" onClick={go('list')}>Tìm biển hợp mệnh</Button>
        <Button variant="outline" size="md" onClick={() => { try { navigator.clipboard.writeText(window.location.href); notify('Đã sao chép liên kết'); } catch { notify('Không sao chép được'); } }}>Chia sẻ liên kết</Button>
      </div>

      {related.length > 0 && (
        <section aria-label="Bài viết liên quan" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
